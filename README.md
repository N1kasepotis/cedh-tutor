...
- **两卡组合技识别：Commander Spellbook 快照**（`config/spellbook-combos.js` + `utils/spellbook-combos.js`）。此前只认手工维护的 `KNOWN_COMBOS` 42 条，牌表里绝大多数两卡组合技一条都识别不出来——这正是「牌组里有组合技却没被认出来」的直接原因。现在补上长尾：**3,015 条**两卡组合技，涉及 1,831 张牌，78.9KB（卡名去重存一份表，配对只存 36 进制下标；直接存名字要 200KB 以上）。**收录范围**＝「产出无限循环」∪「档位 ≥4」，剔除含禁牌的，并**剔除已被 `KNOWN_COMBOS` 覆盖的配对**——手工那份有更好的中文与家族归并，让它继续赢，也避免同一套牌被报两次（有测试守着两边不重叠）。**档位判断信它的**：variant 上的 `bracketTag` 不是「这个组合技多强」，而是「含有它的牌组会被估到哪一档」，由 Spellbook 后端的 `estimate_bracket` 从速度、是否真两卡、产出是否致胜算出来（MIT 开源，规则可查）；字母映射抄自它自己的模型定义：`R=4 S=3 P=3 O=2 C=2 E=1`，`B` 是禁牌。**下限规则**：`max(它给的档位, 3)`——官方 Bracket 2 的定义里明写「没有两卡无限组合技」，所以只要存在就至少是 3；3 与 4 之间的分寸交给 Spellbook，那正是它比手写规则准的地方。**证据收敛成一条**：一副 cEDH 牌能命中七十多组，逐条铺开会把判定依据整栏冲垮——只点名两组、其余用「另有 N 组未列出」带过。这一页其余证据正文中位数 31 字、最长 70 字，这条控制在 150 字以内（有测试卡着）。**双面牌两种写法都索引**：Spellbook 与 Moxfield 都写全名 `A // B`，但有的导出器只写正面名，只认一种会漏掉那一半用户。**归一化规则由调用方注入**，匹配器不自带一份——全项目只能有一条 `canonicalCardKey`，两处各写一套迟早对不上，而对不上的表现是「组合技静默漏检」。更新流程＝跑 `node scripts/build-spellbook-combos.js`（会联网并自检）→ 重新提审。不跑也不会坏，只是漏掉上游新增的组合技。
