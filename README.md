...
| **首页序号底版** | 真机看 60px 点阵巨号：边缘是硬像素块而不是发虚（发虚说明字体没加载上、回退到系统字体了，`wx.loadFontFace` 的失败是静默的），切口贴着发丝线、上方留白随行高变化 |
