...
| **首页序号底版** | 真机看超宽体巨号：① 八行都填满各自的上下缘、被裁比例看着一致（不该有的行满、有的行只剩一条带）② 左缘与中文对齐、右端没被切出竖边 ③ 字面是 Archivo Black 而非系统回退（`wx.loadFontFace` 的失败是静默的）④ 压在其上的中文与副标题仍清晰 |
