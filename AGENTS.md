...
- UI reviews must account for WeChat `style: v2` button defaults, including the 184px normal width and high-specificity disabled styles. A browser harness without those defaults can miss text stacking and white disabled buttons. Check narrow screens and enlarged text; distinguish native compilation and browser simulation from actual WeChat rendering. See `docs/planechase-atlas-design.md` for the confirmed incident.
