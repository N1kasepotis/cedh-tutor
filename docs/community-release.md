...
2026-09-14 第三版改为“暗色收藏档案 × 玩家批注”：署名是最大的字，下接“三张牌认识我”和玩家自选的标签；三张牌之间只用细线分隔，完整卡面按原比例加投影，短评比牌名更大更亮，版本、编号和语言写在卡图下方；强调色取自第一张卡画；小程序码、邀请语、版权与画师署名移到底部参与区。画布高度按内容计算，导出按画布尺寸输出。设计取舍见 `docs/community-design-review.md`，标签的部署状态见下文“待部署：名片标签”。
...
名片契约新增玩家自选标签 `tags`（对局强度、套牌速度、干扰对手三项的选项序号，每项都可以不填），`cloudfunctions/community/contracts.js` 已同步。线上 `community` 仍是 `5410fdd` 的代码：它的 `passport()` 只保留署名、套牌名和三张牌，发布时会丢掉标签，但不会报错；本机草稿和名片海报照常显示标签。用户确认后重新部署 `community`，牌友打开分享名片才会看到标签。
