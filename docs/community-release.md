...
- 云函数已通过官方 CLI 部署；早期云端共享模块加载错误已通过部署根目录布局修正。当前真实调用返回服务错误，直接核查确认 `community` 集合尚不存在（`DATABASE_COLLECTION_NOT_EXIST`）。因此尚未完成真实双用户投票、分享、审核和确认验收。
- 名片图片导出已到达真实卡图下载步骤，但微信返回 `url not in domain list`；须配置 downloadFile 域名 `https://cards.scryfall.io` 后重测。
- 实际云函数仍为默认 3 秒超时；需调整到 30 秒并复查。后台隐私声明与相册授权流程尚待配置后真机验收。

这些阻塞属于交付状态的一部分，不能用本地测试通过、云部署成功或开发版上传成功替代。
