...
本轮在原生小程序中加入名片、名片对照、禁牌表、牌桌约定、调牌记录和开手讨论；赛后复盘扩展现有 tracker。保持暗色桌面工具的配色、按钮和间距，主界面只增加一个统一入口。未引入 React、独立用户账号体系、强制微信头像授权或手机号。
...
本机 `playerStudio` 保存名片、个人表态、最近 20 个牌桌和最多 100 条换牌记录。换牌观察以建立记录时的 match ID 集合为基线；删除已有对局会影响展示的新增场数。原套牌删除后保留调牌记录，并明确标注。
...
- 已按产品要求移除举报入口及云端 `report` 操作；旧操作返回 `INVALID_INPUT`，不会创建举报记录。内容审核及作者撤回继续有效，现有历史数据不做破坏性清理。
...
- 云函数 `community` 状态为 Active、30 秒超时，实际客户端 `poll` 读取成功。独立测试项目通过真实按钮完成名片发布，测试分享已撤回，原草稿已恢复。
- 实际 DevTools 项目中，“保存到本机”能写入空草稿及完整草稿。代码另修复了旧战绩读取异常连带阻断玩家名片初始化的问题，保存成功在操作区显示反馈。
- 实际项目的 `wx.getImageInfo` 返回 `url not in domain list`，关闭重开项目及清理网络缓存后仍存在。相同 AppID 的独立项目能下载同一张卡图并生成海报；加入新查询参数避免复用图片缓存后，两个项目结果仍不同。两边配置均启用 `urlCheck`，不能用独立项目成功替代实际项目或真机验收。
- 图片流程新增下载、画布获取和导出的超时恢复，并将域名错误与一般网络错误区分。失败提示放到操作区并显示 Toast；未选齐三张牌时可点击按钮查看提示。
- 本地 339 项测试、128 个 JavaScript 语法检查和严格推荐诊断通过，23 个 WXML 与 29 个 WXSS 原生编译通过。补充布局检查在 320、375、430px 与 135% 字号下未发现新增反馈和按钮的横向溢出。
- 手机相册实际落盘仍需真机验证。“保存到本机”是草稿保存，不是相册保存；后者依赖海报生成和用户相册授权。

## 2026-09-14 实际项目恢复验收

9 月 13 日记录的实际项目下载阻塞已解除。实际 `xcx` 项目的详情 → 项目配置中，downloadFile 仍显示“未设置”；点击“域名信息”旁的刷新后，显示已在后台配置的 `https://cards.scryfall.io`。运行进程仍保留旧配置，完整退出并重开开发者工具后，`wx.getImageInfo` 实际下载成功。没有关闭 `urlCheck`，没有重置用户存储，也没有改动服务器域名权限。

在实际 `xcx` 项目、基础库 3.17.3 中，通过真实按钮验证：

- 分别搜索并选择 Sol Ring、Brainstorm、Swords to Plowshares 的实际印刷版本
- 保存空草稿、完整草稿与重开恢复
- 全卡名片与卡画模式图片生成
- 点击“保存到相册”，完成开发者工具的本地保存对话框，保存出 750 × 1200 PNG（372,109 字节），成功回调且按钮恢复可用
- 生成分享名片并通过云函数读取三张牌内容，测试结束撤回分享并恢复原草稿

本次处理是开发者工具配置缓存恢复，不需要修改或重新部署云函数。手机微信的首次隐私授权和相册落盘仍由真实手机验收；以上本地图片保存结果不作为手机相册成功的证据。

## 2026-09-14 玩家名片调整

用户已确认此前手机生成与保存成功。名片编辑页移除主将／套牌选择，每次编辑同一份玩家名片；既有玩家草稿优先，没有时带入选牌最完整的旧套牌草稿。旧数据保留，历史分享链接不改写。对局约定与调牌记录仍使用原有套牌数据。

名片海报重绘为 900 × 1440，三段金色、青灰、陶粉色标对应三种选牌理由。全卡模式展示完整印刷卡面，卡画模式铺满真实卡画；牌名按实测宽度缩放，短评最多三行，版本信息和画师署名保留。下载、超时恢复、隐私与相册授权沿用已验证链路。

2026-09-14 再次简化：去掉三段色条、右上角大号“03”和每段序号，改为冷黑底、银白与冷灰文字层级，钴蓝只保留标题块下一条短线。全卡模式把完整牌面等比放在左侧，卡画模式改为左侧画带，不再铺满整块面板；没署名时不再重复“三张牌认识我”。卡图与画师署名、下载与导出链路不变。
...
2026-09-14 开手题 ID 整体更换，旧的三道自拟题不再接受投票。云函数按 `catalog.js` 判定题目是否存在，需要用同步后的副本重新部署 `community`，新题才能投票；部署前服务端不认这些题号（返回 `INVALID_VOTE`）：练习页不显示社区票数，也不报错；禁牌表遇到未开放的轮次显示“这一轮投票暂未开放”。

## 2026-09-14 云函数重新部署

用户确认后，用官方 DevTools CLI 从实际 `xcx` 项目根目录部署 `community`（源提交 `5410fdd`，环境 `cloudbase-d6gvqujh4d0efe37f`，启用 remote npm install）：

- 部署前：状态 Active、超时 30 秒、运行时 Nodejs16.13
- 部署结果：成功，上传 7 个文件、11.4 KB；部署后仍为 Active、30 秒、Nodejs16.13，已有函数的超时设置没有被覆盖
- 核对：用 CLI 把线上函数下载回来，7 个文件与本地提交逐个比对一致（统一换行符后）；线上 `catalog.js` 含新开手题 ID，不再含旧的三道自拟题
- 未验证：没有从小程序实际发起读票或投票调用；需在开发者工具或手机打开“这手留不留”，投一票后应出现社区比例
- 运行时仍是 Nodejs16.13，低于上文建议的 Node.js 18；本次未改动，需要时在云控制台调整

本次只部署云函数，没有上传小程序开发版，也没有提交审核或发布。
